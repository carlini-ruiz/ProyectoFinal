...
export default class Pantallab extends Component {
  ...
  componentDidMount() {
    ...
    xhttp.open('GET', 'https://prograinter-proyecto.000webhostapp.com/HDCMostrarDatos.php', true);
    xhttp.send();
  }
  render() {        
    const _this = this;
    const getItem = (id, nombre1,telefono1,tipo_equipo1,marca1,modelo1,numserie1,falla1,accesorios1,dia_cita1)=>{
      console.log(id);
      _this.props.navigation.navigate("id",{id:id,nombre:nombre1,telefono:telefono1,tipo_equipo:tipo_equipo1,marca:marca1,modelo:modelo1,numserie:numserie1,falla:falla1,accesorios:accesorios1,dia_cita:dia_cita1})
    }    
    const renderUser = ({item}) => {
      return (        
        <View style={{margin: 5, borderWidth: 1, padding: 10}}>
          <TouchableOpacity onPress={()=>{ getItem(item.id, item.Nombre,item.Telefono,item.Tipo_Equipo, item.Marca,item.Modelo,item.NumSerie, item.Falla,item.Accesorios,item.Dia_Cita)}}>           
